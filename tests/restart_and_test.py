"""Kill old backend (except self), restart with fixed code, run tests."""
...
def kill_old_backend():
    """Kill any python process on port 8080 except ourselves."""
    import socket
        ...
        for line in result.stdout.split("\n"):
            if ":%d" % PORT in line and "LISTENING" in line:
                ...
                if pid.isdigit() and int(pid) != MY_PID:
                    print("[CLEANUP] Killing PID %s on port %d" % (pid, PORT))
                    subprocess.run(["taskkill", "/F", "/PID", pid], capture_output=True)
                elif pid.isdigit() and int(pid) == MY_PID:
                    print("[CLEANUP] Skipping self (PID %s)" % pid)
    except Exception as e:
        print("[CLEANUP] netstat failed: %s" % e)


def start_backend():
    """Start the backend and wait for readiness."""
    print("[START] Starting backend with fixed code...")
    ...
    for _ in range(60):
        time.sleep(0.5)
        try:
            r = urllib.request.urlopen("http://localhost:%d/health" % PORT, timeout=3)
...
def run_chat(provider, model, message, history=None):
    ...
    try:
        with httpx.Client(timeout=120) as client:
            with client.stream("POST", "http://localhost:%d/chat" % PORT, json=payload) as resp:
...
def main():
    ...
    for provider, model, label in providers:
        ...
        # Count tool execution signals
        tool_signals = resp.count("tool") + resp.count("executing") + resp.count("Calling")
        multi_step = tool_signals >= 2 or ("notepad" in resp.lower() and "hello" in resp.lower())
        tag = "[OK]" if multi_step else "[FAIL]"
        print("      %s (%.1fs, signals=%d): %s" % (tag, elapsed, tool_signals, resp[:300]))
        results.append(("%s multi-step" % label, multi_step, elapsed, tool_signals))
    ...
    for entry in results:
        name, ok, elapsed = entry[0], entry[1], entry[2]
